...
log("MAIN CONFIGURATION LOADED")

-------------------------------------------
-- USER CONFIG
-------------------------------------------

-- It is strongly advised that you use this file as a reference-only and introduce config changes in rc.lua
-- Persistent entries are marked with `*` and changing them at runtime might cause side-effects or not be effective
...
local config = {
	...
	screens = {
		...
		["*"] = {
			...
			-- TODO: Awful taglist filter, possible keys: `all`, `noempty`, `selected`, defaults to `all` if not set
			-- this actually changed, libraries can't now return that shit
			...
			-- Whether to use focus applications opened using tag's `spawn` logic, `false` by default
			spawn_focus = false,
			
			-- Whether to raise applications opened using tag's `spawn` logic, `true` by default
			spawn_raise = false,
			...
			tags = {
				...
				[2] = {
					name = "im2",
					icon = "/home/name/.config/awesome/ness/img/tag/1.svg",
...
function config_mt.__call(self, tbl)
	util.table.merge(config, tbl)
	config.apply()
